    ...
    <el-container style="margin: 15px 20px 20px 20px">
      <el-main>
        ...
        <el-table :data="data" style="width: 100%" :loading="loading" border>
          ...
          <el-table-column prop="voucher_id" label="代金券ID" align="center">
            <template #default="scope">
              {{ scope.row.voucher_id }}
            </template>
          </el-table-column>
          <el-table-column
            prop="voucher_name"
          ...
          >
            <template #default="scope">
              {{ scope.row.voucher.voucher_name }}
            </template>
          </el-table-column>
          <el-table-column
            prop="get_time"
          ...
          >
            <template #default="scope">
              <span v-for="(item, index) in getTypeOptions" v-bind:key="index">
                <el-tag
          ...
          >
            <template #default="scope">
              ...
              >
                <el-tag
                  v-if="scope.row.use_status == item.value"
                  type="success"
                  effect="dark"
                  >{{ item.label }}</el-tag
                >
...
<script>
import { fetchVoucherGetLogPage } from "@/api/voucher_get_log.js";
...
export default {
  ...
  methods: {
    handleListData() {
      this.loading = true;
      fetchVoucherGetLogPage(this.listQuery)
        .then((res) => {
