    ...
    <el-container style="margin: 15px 20px 20px 20px">
      <el-main>
        ...
        >
          ...
          >
            <template #default="scope">
              <div v-if="scope.row.valid_time_type == 2">用户领取时间</div>
              <div v-else>
...
export default {
  ...
  methods: {
    handleListData() {
      this.loading = true;
      fetchVoucherPage(this.listQuery)
        .then((res) => {
    ...
    handleUpdate(data) {
      ...
      this.form = {
        ...
        valid_time_type: data.valid_time_type,
        valid_day: data.valid_day,
        use_type: data.use_type,
        status: data.status,
        get_begin_time: data.get_begin_time,
        get_end_time: data.get_end_time,
        use_begin_time: data.use_begin_time,
